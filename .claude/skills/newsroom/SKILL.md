...
> **選題鐵則｜先問「有多少人搜」（判準零）**
>
> 站上已排到第一頁的 190 個字，平均每個字只有 6.9 次曝光；523 個 query 裡 86% 只有 1~5 次曝光。
> 問題不是排不上去，是**選的題沒人搜**。別選只有業內人士會用的術語（「衰弱前期」「功能健康評估」），
> 改用一般人會打進搜尋框的說法。**寧可量大排第 10，不要量小排第 1。**
>
> **標題鐵則｜主標瞄準概念字，不是專有名詞（動筆前先定案）**
>
> GSC 90 天實測：站上事件型標題**排名很好（pos 4~10）但幾乎沒有曝光**——`figure 03 bmw` 11 次、`fable 5` 3 次、`work iq` 2 次，因為這些字只在事件當週有人搜。同期曝光最高的是〈Passkey 通行密鑰**是什麼**：無密碼登入的**運作原理**與台灣導入現況〉（177 次，全類第一），唯一一篇概念解釋文。**差 16 倍。**
>
> 所以下標前先講清楚「這篇要吃哪個字」，並過三個檢查：
> 1. 那個字**不能只是專有名詞**（產品名／公司名／型號／版本號）。要是概念詞或問題：`人形機器人 產線`、`ai 模型 選型`、`髖關節痛 看哪一科`、`美顏針 禁忌`。
> 2. **三個月測試**：三個月後還有人搜嗎？答不出「會」就重擬標題。
> 3. **主標必須含那個字的核心詞**。事件寫進副標或第一段當引子沒問題，主標不行。
>
> 例：〈SK 海力士砸 19 兆韓元蓋封裝廠〉→〈HBM4 為什麼把戰場推到封裝？〉。
> 同一篇稿、同一批素材，只換瞄準點。
>
> 若工單帶了 `targetQuery` 欄位，直接用它；沒有就自己定一個再動筆。
...
5. 封面圖：同法 `node scripts/get-image.mjs --topic "<本篇主題>" --context "<摘要>" --query "<英文關鍵字>" --out public/covers/<slug>.webp`（封面多為概念圖，預設走圖庫優先；若封面主體是人物才加 `--people`，並同樣帶 `--caption/--alt/--article-context`）。frontmatter 填 `coverImage: "covers/<slug>.webp"` + `coverAlt`；**若回傳 `mode:"stock"`，另填 `coverImageCredit: "<回傳的 credit>"`**（圖庫署名）。
   ...
   - **封面一律橫式、寬 ≥1200px**（Google Discover／Top Stories 大圖版位門檻，規格 SOT＝`scripts/lib/cover-spec.mjs`）。get-image 對 `covers/` 路徑會機械把關：不合規格的圖庫候選自動淘汰、全滅就換一組 `--query` 再取；**不要**手動複製別的圖檔來當封面（會被配圖 gate 以同一規格擋下）。
6. frontmatter：`slug / title / description / category / subcategory / tags / highlights / risksAndLimits / references / author: "lightman" / sourceType: "editorial" / disclaimerType`。先設 `status: "draft"`（排程待步驟四）。
   - **`risksAndLimits`（文末「風險與限制」區塊）**：2～4 條、每條 ≤60 字，寫讀者照這篇行動前該知道的邊界（研究樣本或設計限制、結論適用族群、資料時效與地區限制、法規可能變動、尚無定論之處）。**要具體可查證**，不要寫「僅供參考」「請諮詢專業人員」這類空話（免責已由 `disclaimerType` 處理）。純事實整理確實無實質限制可寫就不要填，不要硬湊。單一定義在 `scripts/lib/risks-prompt.mjs`（七條自動產線共用同一份），為什麼＝[`docs/lessons/provenance-disclosure.md`](../../../docs/lessons/provenance-disclosure.md)。
   - ⚠️ `sourceType` 只能填 `src/content.config.ts` 的 enum 合法值（`editorial` / `author` / `contributor` / `expert` / `press-release` / `sponsored` / `partner` / `wire`）。本行原本寫的 `"ai-assisted"` **不是合法值**、會讓 build 直接失敗，2026-08-01 已更正為 `editorial`。
...
7. **去 AI 腔文風複查**（產檔前必過，違反即改；`node scripts/check-content.mjs <檔>` 是統一內容守門引擎，兩級判定：ERROR 硬 tell 會 exit 1 擋 build、WARN 軟訊號單檔跨 ≥3 層才升 ERROR，語氣類多屬 WARN 仍靠這份清單自檢）：
   ...
   - 禁 AI 排比／下定義套語：「不是X，而是Y」下定義、「不僅…更/還…」「不只是…而是/更是…」「並非…而是…」排比、「從 A 到 B 到 C」排比。**（`不是X而是Y`、`不只是…更是/而是`、`並非…而是` 均為 check-content 核心 ERROR，命中即擋 build）**
   - 禁轉場／收束套語：「值得注意的是」「值得一提的是」「換句話說」「事實上」、自問自答「那麼…呢？」；空泛收束「總的來說／綜上所述／總而言之／歸根結底／整體而言」。
   - 禁拔高與開場公式：「真正的問題／關鍵是…」拔高、「隨著…的發展/普及」「在…的今天」開場公式、「至關重要／不可或缺／舉足輕重」。
   - 禁模糊引用：「研究顯示／有研究指出／專家認為／學者認為／普遍認為」——要嘛當下附**具體可點的來源超連結**、要嘛不寫。**（check-content 核心把模糊引用列 ERROR）**
   - 禁模板化第一人稱開場：以「我」起句、「老實說／老實講」、「最近有讀者／客戶／朋友…」這類套版開場。
