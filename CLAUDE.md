...
> **多分類自動內容（國際/生活/運動）**：科技以外的頻道**各有各的萃取邏輯與來源**（GDELT、政府開放資料、各地警局、學生賽事投稿…），**不是同一套雷達**。完整 cron 總表、各頻道來源/上線方式/Slack 行為、並發保護，見 [`docs/SERVER_HANDOFF.md`](./docs/SERVER_HANDOFF.md) §子專案 3；設計脈絡見記憶 `new-verticals-automation-plan` / `intl-desk-gdelt` / `slack-appi-news-workspace`。下面這條是科技日更（最早的那條）。
...
- **所有 publisher-checkout cron 必用 `flock -w 1800 /tmp/appi-publisher-cron.lock` 序列化**：每支都 `git reset --hard origin/main`，並發會互洗未提交工作。新增 cron 一定沿用此鎖樣板（見各 `scripts/cron/*.sh`）。
