...
```
tech-radar（cron 一天三次）→ 發候選題到 Slack（帶「我要寫這題」按鈕）
...
| 元件 | 路徑 / 識別 | 角色 |
|---|---|---|
| 選題雷達 | `.claude/skills/tech-radar/`、`scripts/cron/tech-radar.sh` | 只產 tech 候選；cron UTC 21:20 / 03:11 / 10:18（台北 05:20 / 11:11 / 18:18） |
...
- **所有會 `git reset --hard origin/main` 的 publisher-checkout cron 必用 `flock -w 1800 /tmp/appi-publisher-cron.lock` 序列化**：並發會互洗未提交工作。新增這類 cron 一定沿用此鎖樣板（見各 `scripts/cron/*.sh`）。例外：純資料腳本（如 `indexing-submit.sh`，不碰 git 工作區）不需 flock，見 `docs/SERVER_HANDOFF.md` §cron 總表。
