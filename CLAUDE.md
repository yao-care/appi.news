...
| 論壇選題雷達 | `scripts/lib/forum-signals.mjs`（純資料）＋`scripts/forum-radar.mjs`＋`scripts/cron/forum-radar.sh` | **每小時**掃 PTT 白名單看板 → 跨 6 分類候選 → Slack 各分類台（帶「我要寫這題」按鈕）。**抓取／政治過濾／去重全是純 node，沒有新熱題就 exit 0、完全不喚 Claude**，所以每小時跑也不吃額度視窗。改看板或門檻＝改 `BOARDS`。**政治排除三層**（看板白名單／標題關鍵字／LLM 判斷），第三層不可省——PTT 索引標題會被截斷，關鍵字擋不完。細節見 [`docs/SERVER_HANDOFF.md`](./docs/SERVER_HANDOFF.md) §cron 總表 |
| Slack server | `scripts/slack-actions-server.mjs`、pm2 `appinews-slack-actions` | 收按鈕事件觸發產文，回報摘要/重點/預覽連結；也收 GitHub webhook（dev-bot 開發、`article-draft` 寫作）。**事實稿候選（含 health/finance）也掛按鈕**：kind=factual 時 modal 的看法改選填、validateJob 與 newsroom-write 皆帶 `--allow-any-category`，產**待審草稿**而非直接上線（放寬的是「誰能開單」不是「誰能上線」） |
...
| 科技台自動線 | `scripts/tech-desk.mjs`＋`scripts/lib/tech-desk.mjs`＋`scripts/cron/tech-desk.sh` | 科技的**自動產文**線（有別於只產候選的 tech-radar）。兩條 track：`editorial`（APPI 編輯部，事實型概念解釋）／`lightman`（張饒輝《AI 醫療現場》，AI×健康觀點）。cron **每天只跑一條、日期奇偶輪替**（避免單日雙倍吃額度）。選題判準與標題鐵則寫在 prompt 裡，GSC 訊號 fail-open。為什麼＝[`docs/lessons/query-targeting-event-vs-concept.md`](./docs/lessons/query-targeting-event-vs-concept.md) |
...
| 急性症狀衛教 | `scripts/lib/acute-care.mjs`（題目表＋`BOUNDARY` 寫作界線）＋`scripts/acute-care.mjs`＋`scripts/acute-care-batch.sh`＋`scripts/acute-care-audit.mjs` | **清單驅動、非 cron**（2026-08-03 一次性跑完 27 題）。要再開題就往 `TOPICS` 加，跑 `acute-care-batch.sh`。醫療界線寫在 `BOUNDARY` 常數、原文進 prompt，要收緊改一處即可。合規用 `acute-care-audit.mjs` 機械驗七項，不靠人抽驗。為什麼＝[`docs/lessons/acute-care-line-traps.md`](./docs/lessons/acute-care-line-traps.md) |
