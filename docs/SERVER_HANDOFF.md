...
- **內容線排程刻意攤開，別再併回早上（2026-07-03；2026-07-20 更新；2026-07-26 加影片線）**：**實際時間一律查 crontab，本檔不列**（列了必然過期）。原則如下。科技台那條的由來＝科技原本只有選題雷達（產候選→Slack→人挑），沒有自動產文線；它排在當時剩下的最大空窗，間距已不到 ≥5h——**因為 24 小時已排滿**。該線**每天兩條 track 都跑**（editorial＋lightman；2026-08-22 站長裁示解除原本的日期奇偶輪替——輪替是為 claude-appi 的 5 小時共用視窗設的，寫作 2026-08-22 全面改 codex 後理由消失；若寫作引擎回 claude 池要先恢復輪替）。歷史脈絡：claude-appi 的 session 額度是**每 5 小時一個共用視窗**，曾經全擠在同一個視窗搶額度，排最後的那條每天餓死（先撞 weekly limit、再 session limit）——這條原則仍適用於所有還在喚 claude 的非寫作 cron。原則：任兩條相隔 ≥5h 或跨 04:30 reset 邊界，一個視窗最多落一條。**⚠️已知例外（站長 2026-07-20 指定）**：連假優惠與警消落在同一個視窗——但連假優惠**只有臨近國定連假才實發**（平日靜默），撞期風險僅限連假前數日，故接受。要再調時間，日更線（焦點/警消/國際/雷達/便民）務必維持 ≥5h 間隔，別看「都在白天比較整齊」就併回去。（另一個常態吸額度點是 agent.writer 每小時 :40 的 cron-write，跨專案，未動。）
