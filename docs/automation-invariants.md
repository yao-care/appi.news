...
- **非寫作呼叫維持 `claude-appi`**（`CLAUDE_CONFIG_DIR=~/.claude-appi`）：查核 gate（viewpoint／國際線 triage／論壇政治過濾）→ `haiku`；選題雷達、週報、大腦體檢、AEO 探針、`get-image` 配圖展開與視覺自檢 → `claude-sonnet-5`。每個 `claude-appi -p` 呼叫**必帶 `--model`**，不帶就吃全域預設 Opus、燒爆週用量上限（出過事 → [`automation-model-and-account-split.md`](./lessons/automation-model-and-account-split.md)）。**每日大腦層（seo-ops）不在此範圍、維持 claude**（站長同日裁示）。
