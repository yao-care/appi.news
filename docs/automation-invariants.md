...
- **產圖全走 codex**（站長 2026-08-22 三度裁示，最終版＝**100% codex、無備援**）：**生圖唯一路徑＝codex 原生 `image_generation` 工具**（`lib/ai-image.mjs` 的 `generateViaCodex`，1536x1024 橫式）。**不得退回 gpt-image-2 API／worker**——codex 失敗即 throw，由配圖 gate 照紅線中止不發、改動留工作區待補；worker 現在只剩圖庫搜尋（`/stock-search`）用途。已知殘口＝/admin 編輯器的人工生圖鈕（瀏覽器打 worker `/generate-async`，瀏覽器跑不了 codex CLI；是否停用待站長裁示，見 2026-08-22 對話）；prompt 展開與圖庫/生成圖視覺審查走 `runWriterOnce`（`lib/writer-cli.mjs`，read-only 沙箱、`-i` 附圖、prompt 一律走 stdin——`-i` 是變長參數會吞位置參數，2026-08-21 踩過）。
