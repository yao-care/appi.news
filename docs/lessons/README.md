...
| [duplicate-topic-gate.md](./duplicate-topic-gate.md) | `髖關節痛` 被自家 5 個 URL 瓜分全卡 pos 76-83：去重下在選題端（14 天窗、單一產線）蓋不到跨月與跨產線的重複，改下在寫入端；另查出 131 個 meta-refresh 轉址殘骸仍吃 15% 站台曝光，「轉址已處理」≠「權重已傳遞」；附釘目標字前先用 GSC 驗曝光（高曝光≠高機會），與「換掉目標字卻沒回頭盤誰在讀那份清單」害 GEO 體檢連 12 天報同一句假故障 |
| [bulk-stock-article-editing-traps.md](./bulk-stock-article-editing-traps.md) | 一次批次改幾百篇存量會踩的四類 build 炸點：updatedDate 重複 key、未來排程稿不可加 updatedDate、HTML 段內 Markdown 連結不渲染（且檢查分檔漸進爆）、觸碰任一行整檔脫離 grandfather；每輪收尾照機械檢查清單跑 |
...
| [google-indexing-api-gray-area.md](./google-indexing-api-gray-area.md) | 「有 GSC key 就能催收錄」是誤解；Indexing API 對新聞站非官方、200 不保證收錄；配額是 per-GCP-專案故 appi 要自己一把、換金鑰只換一半害 GSC 靜默斷線兩天（服務帳號授權是「按 Google 產品」給的，要逐個 API 驗）；檔頭註解承諾的 key fallback 程式根本沒寫（宣稱容錯的註解旁邊要看得到那個 `if`），離開伺服器讀不到 GA/GSC 時唯一合法來源是 `data/seo-daily/` |
...
| [deploy-cadence.md](./deploy-cadence.md) | 部署連續逾時不是站台太大（只佔上限 1/3）而是一天推 200 次被節流；卡在 deployment_queued 就代表 GitHub 還沒開始處理。拿掉 push 觸發改 15 分鐘排程，並加變動偵測——只壓尖峰不看總量會把每日次數從 19 放大到 96 |
...
| [reader-index-and-beacon-contract.md](./reader-index-and-beacon-contract.md) | LINE 官方帳號要的索引與閱讀量測：契約正本在對方 repo，欄位改壞只會讓對面靜默降級不會報錯；索引取「各分類最新 40 篇聯集」而非全站最新 N 篇（否則 focus 整個消失）；捲動深度不可在 rAF 連續取樣取最大值（延後載入的圖讓早期 scrollHeight 偏小、比例灌水），改成送出當下用最終高度算一次；停留計時用真值判斷時間戳會被 `performance.now()===0` 吃掉整段，靠 node:vm 假 DOM 模擬才抓到；**追記**：文章頁 mobile 暖跑 TBT 140–215ms 的來源是 GA4 gtag.js 不是新程式，附「PSI 暖不起來時怎麼量」的兩段式方法（後續已拆掉 gtag，見 [ga4-beacon-instead-of-gtag.md](./ga4-beacon-instead-of-gtag.md)）|
| [ga4-beacon-instead-of-gtag.md](./ga4-beacon-instead-of-gtag.md) | gtag.js 是文章頁唯一的主執行緒阻塞來源（A/B：TBT 中位 406→30ms、TTI 從鎖死 ~8s 掉到 ~2s），且它已在 `load`+idle、再延只會把長任務推到讀者捲動當下並讓 TTI 更晚＝沒有剩餘空間；改成 inline 自送 `/g/collect`。**參數集用 headless Chrome 攔真 gtag 封包比對出來**（`content_group` 是 `ep.content_group`，`gtm`/`gcd` 那票不送也收得到），攔截時必須 abort/respond 否則本機測試會灌髒資料進正式資源；`cid` 沿用 `_ga` cookie 保住使用者連續性；**GA4 realtime API 有幾分鐘延遲，150 秒查不到不等於沒收到**，且一次 page_view 會生成 3 個事件（含 session_start／first_visit）|
...
| [provenance-disclosure.md](./provenance-disclosure.md) | 做了審閱卻沒人看得見：`reviewedBy`/`factCheckedBy`/`editor` 三欄存在但使用數 0，`DISCLOSURES` 又把 editorial/author 設成 `null`，570/574 篇前台查不到任何產製資訊；根因是「沒有前台出口的欄位不會有人填」，解法＝分類→審閱者單一來源＋渲染退回機制（產線不必逐支改）＋揭露句重點放流程與具名審閱者而非「用了 AI」 |
...
| [hreflang-mirror-return-links.md](./hreflang-mirror-return-links.md) | 多語鏡像站單方面掛 hreflang 指回本站，本站不回指＝整組宣告被 Google 忽略（成對協定只做一半等於沒做）；對照表 `src/data/mirror-alternates.json` 由鏡像產線自動同步，不要手改 |
| [mechanical-backfill-traps.md](./mechanical-backfill-traps.md) | 存量批次回填（內鏈 490 篇、常見問題 475 篇、7 個主題中樞）踩到的六個坑：lint 判準比渲染端窄導致工作清單騙人；「同分類同標籤」不等於同一題（要用 TF-IDF 餘弦，標題 2-gram 會被模板句型污染）；批次改檔只能新增獨立行否則存量舊句被 AI 味 gate 攔下；禁止模型放連結會把它推去寫「研究顯示」正好撞模糊引用；自動產生的 id 不可以有編碼備案（會產出 topic-k34gad 這種網址）；地理標籤要當面向排除，內聚度分不開它與真主題 |
