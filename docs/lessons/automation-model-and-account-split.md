...
## 續（2026-07-28）：雙帳號分工取消，全站統一 `claude-appi`

站長裁示：不再區分「dev 用 `claude`／營運用 `claude-appi`」，**互動開發與所有自動化都用 `claude-appi`**。本篇前段關於「帳號切換」的歷史仍是原始脈絡，但**分工本身已不存在**，正本規則見 [`docs/automation-invariants.md`](../automation-invariants.md) §帳號與模型。

本篇其餘教訓**完全不受影響、且份量更重**：

- 帳號合一後**用量池只有一個**——互動開發吃掉的額度會直接壓縮自動產線的配額，撞週限「整批空跑」的劇本（上一節）更容易上演。大批量互動作業前先確認當日產線需求。
- 憑證同樣合一：`~/.claude-appi/.credentials.json` 一失效，互動與 cron 一起啞（2026-07-26 已發生，四站同時掛，無備份，只能重新 `/login`）。
- `--model` 鐵則不變：全域預設仍可能是 Opus，不帶就燒額度。
