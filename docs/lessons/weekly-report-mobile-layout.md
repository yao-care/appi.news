...
## 追記（2026-08-08）：Slack 已經有原生 table block，「別用假表格」不等於「不能用表格」

做主題追蹤頻道時要排多欄數據，第一反應是照本篇結論改成兩行制。實際查了 Slack Block Kit 文件才發現**現在有原生 `table` block**（`chat.postMessage` 的 `blocks`／`attachments` 都收，上限 100 列／20 欄／全表 10,000 字元；儲存格型別 `raw_text`／`raw_number`／`rich_text`，`column_settings` 可設對齊與是否換行）。它由 Slack 自己負責響應式排版，不會有「`|` 對齊在窄螢幕垮掉」的問題——本篇真正的教訓是**不要用管線與空白假裝對齊**，不是「表格不能用」。

實作要點（`scripts/lib/topic-tracker.mjs`）：

- 儲存格一律 `raw_text` 放格式化好的字串，**不要猜 `raw_number` 的欄位名**；要連結就用 `rich_text` + `link` 元素（`:shortcode:` 在 `raw_text` 會原樣印出，emoji 要放 Unicode 字元）。
- **一定要留退路**：工作區若不支援，`chat.postMessage` 會回 `invalid_blocks`；攔下來改發兩行制 mrkdwn，不要讓整批訊息啞掉。
- 一樣**驗不出來就是驗不出來**：本機看 JSON 不算數，實發一則到 dev 頻道用眼睛看（這次就是靠實發才發現「排名 8.4 ▼0.7」的箭頭語意相反——曝光變多是好事、排名數字變小才是好事，同一套 ▲▼ 規則套下去會把進步顯示成退步）。
- 符號代表**好壞、不是數值方向**：本站現在統一「漲/進步用 🔺、跌/退步用 `-`」，跨欄一致。
