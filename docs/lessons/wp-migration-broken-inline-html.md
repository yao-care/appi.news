...
- **D 孤兒 `<h4>` 緊接 `<h3>`**：段落開頭殘留一個通用標籤 h4（「行動步驟」「可執行步驟」「你現在就能做的事」），緊接著才是真正的段落 h3——h4→h3 是層級倒置的遷移殘留。

盤點結果：A 222 處／74 檔、B 68 處／61 檔、C 54 處／37 檔、D 23 處／21 檔。
...
- **D**：`<h4>…</h4>` 後（僅空行/`<img>`）緊接 `<h3>` → 只刪 h4 那行（用 lookahead 斷言後面是 h3、**不消費中間的 `<img>`**，避免誤刪段落圖）。h3 才是真標題，通用 h4 標籤是冗餘。
...
- **未涵蓋的子型態**：全站掃過已無單獨空 `<table></table>` 殘留；其餘畸形（若日後匯入再現）先 `--dry-run` 盤點再決定是否新增規則，別硬塞。
