...
**③ 關鍵字初篩每加一台就要重看一次漏網。** 加了華視、TVBS 之後立刻漏進兩則：「鹽埔鄉長**選戰**掀話題　參選人拋爭取知名**漢堡**進駐」（靠美食詞混進來）、「十分放天燈**空中解體**下火焰雨**遊客**尖叫」（靠景點詞混進來）。`OFF_BEAT` 因此補上選戰／參選／候選人／各級首長，與解體／翻覆／失事／罹難／傷者／送醫／搜救。**新增頻道後務必跑一次 `node scripts/lifestyle-video.mjs` dry-run 看候選清單，別直接讓它自動發。**
