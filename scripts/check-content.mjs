...
// appi.news 特化（從舊 scripts/check-ai-tone.mjs port；核心已有的「不僅…更／值得注意的是／
// 綜上所述／這是一個X也是一個Y／危言聳聽·坦白說」不重複加）：
//   - 破折號：appi 全站硬禁（舊站存量 grandfather 掉），比核心（列 WARN 句式）嚴一級 → 升 ERROR。
//   - 自我辯白式 meta 旁白家族：「先揭露，免得你覺得我在夾帶」上線事件的簽名句，見
//     docs/lessons/ai-tone-gate.md，機械可判、近乎零誤判 → ERROR。
const SITE_ERROR_TELLS = [
  ["破折號（改句號/逗號/冒號或拆句）", /[—―]{1,3}|(?<![-!])--(?!-)/],
  ["自我辯白 meta 旁白「免得你…」", /免得你(覺得|以為|認為|想)/],
  ["自我辯白 meta 旁白「我在夾帶」", /我在夾帶/],
  ["自我辯白 meta 旁白「不怕你笑」", /不怕你笑/],
  ["自證資格式開場「先講清楚我為什麼有資格…」", /先講清楚[：:，,]?\s*我為什麼(有資格|夠格)/],
];
const SITE_WARN_LAYERS = {
  詞彙: [],
  句式: [],
  結構: [],
  // 核心無此條：「其實是同一件事的不同切面」空泛升華（appi 舊 gate WARN）。
  語氣: [["「同一件事的不同切面」空泛升華", /其實是同一(件事|回事)[^。]{0,8}(不同切面|的不同面向)/]],
};
