...
cron_worktree "cited-$beat" "--dev" || exit 0
...
cron_capture --timeout 1800 -- claude-appi --model claude-sonnet-5 -p "/cited-teardown $beat"
if cron_failed "$CRON_LIMIT_RE"; then
  cron_report "--dev" "$(printf '❌ %s（beat=%s）失敗（exit %s，%s）\n%s' "$TASK" "$beat" "$CRON_RC" "$ts" "$(tail -c 400 <<<"$CRON_OUT")"
)"
  exit "$CRON_RC"
fi
...
if ! git diff --quiet -- "$GI" 2>/dev/null || [ -n "$(git ls-files --others --exclude-standard -- "$GI" 2>/dev/null)" ]; then
  ...
  if [ "$pushed" -ne 1 ]; then
    cron_report "--dev" "$(printf '⚠️ %s（beat=%s）檢查表已生成但 push 失敗（%s）——下次會重跑' "$TASK" "$beat" "$ts")"
    exit 1
  fi
