#!/usr/bin/env bash
# 每日 cron：appi.news 維運心跳 → dev 頻道。三則訊息：
#   📊 數據心跳（data-heartbeat.mjs，純讀本地內容存量、無 LLM、必發）
#   🤖 大腦優化（brain-checkup.mjs，claude-appi sonnet 判讀 SEO/內容機會；撞限額退化成只報事實）
#   📊 數據總覽（dashboard-post.mjs，GA 統整:8 區塊中文人流+受眾+漏斗+AEO+連結，純讀 GA、必發）
...
# 3) 📊 數據總覽（GA 統整，最後發 → Slack 最新最顯眼；純讀 GA、必發）
