...
# 兩步各自成敗、各自回報（一步壞另一步照發），故不走 cron_run 的「失敗即結束」，只用外殼的 boot/env。
TASK="維運心跳"
...
if [ "$drc" -eq 0 ] && [ -n "$data" ]; then
  ...
  cron_report "--dev" "$(printf '❌ %s：數據心跳失敗（exit %s，%s）\n%s' "$TASK" "$drc" "$ts" "$(tail -c 300 <<<"$data")")"
fi
...
if [ "$hrc" -eq 0 ] && [ -n "$dash" ]; then
  ...
  cron_report "--dev" "$(printf '❌ %s：數據總覽失敗（exit %s，%s）\n%s' "$TASK" "$hrc" "$ts" "$(tail -c 300 <<<"$dash")")"
fi
