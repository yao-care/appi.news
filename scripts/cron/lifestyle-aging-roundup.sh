#!/usr/bin/env bash
# 每日/每月 cron 進入點：載入金鑰 → 跑 /lifestyle-aging-roundup 技能（樂齡活動 roundup（每月一次））。
# 事實稿走「人工審後發」：技能會產待審草稿並回報 Slack 附「發佈」鈕，不自動上線。
# 本機 cron(Vixie)忽略 CRON_TZ、一律 UTC；以下為 UTC（台北 -8）。
# crontab 範例（台北每月 1 號 09:00 = UTC 每月 1 號 01:00）：
#   0 1 1 * * PUBLISH_ISOLATED=1 /root/appi.news-publisher/scripts/cron/lifestyle-aging-roundup.sh >> /tmp/lifestyle-aging-roundup.log 2>&1
...
out="$(claude -p "/lifestyle-aging-roundup" 2>&1)"; rc=$?
printf '%s\n' "$out"
...
printf '{"text":"⚠️ 樂齡活動 roundup（每月一次） cron 進入點失敗（%s，%s）。請看 /tmp/lifestyle-aging-roundup.log。"}' "$reason" "$ts"   > "/tmp/lifestyle-aging-roundup-fail.json"
node scripts/slack-post.mjs "/tmp/lifestyle-aging-roundup-fail.json" || true
exit "$rc"
