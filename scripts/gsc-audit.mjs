...
//
// 用法：node scripts/gsc-audit.mjs [天數，預設 90]
...
for (const f of readdirSync(ARTICLES_DIR)) {
  ...
  canonical.set(pick('slug') || f.replace(/\.mdx?$/, ''), pick('category') || '?');
}

const slugOf = (url) => decodeURIComponent(url).match(/\/articles\/([^/]+)\/?$/)?.[1];
