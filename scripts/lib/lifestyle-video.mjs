...
/**
 * 組 Claude「挑一支影片 → 交叉查證 → 寫一篇事實整理」的 prompt。
 * @param {Array} candidates 新候選影片（已去重）
 * @param {string[]} recentTitles 近 14 天已發的影片線文章標題（避免撞題）
 */
export function buildVideoPrompt(candidates = [], recentTitles = []) {
  const recent = recentTitles.length ? recentTitles.map((t) => `  - ${t}`).join('\n') : '（近期無）';
  const today = new Date().toISOString().slice(0, 10);
  return [
    '你是 APPI News 生活線編輯。系統訂閱了幾個 YouTube 頻道，下面是今天新出現、通過生活線初篩的影片。你的工作是**挑一支最值得寫的**，把它當**線索**，查證後寫成**一篇**繁中（台灣用語）的事實型整理。編輯部署名、中性語氣、無個人觀點、全自動上架。',
    '',
    ...
    '',
    '【挑選】',
    '- 挑對讀者**真的有用、且查得到公開資料**的生活題：美食餐飲、旅遊景點住宿、健康養生、居家消費、育兒銀髮、在地職人與老店故事。',
    '- 剔除：政治／選舉／社會案件／股市／體育賽事；也剔除純活動花絮、純業配、以及查不到第二來源的獨家軟訊息。',
    '- 一次只寫一支。若候選裡沒有一支通得過上面的 gate，輸出 SKIP。',
    '',
    '【寫作】',
