// 一次性：把用 SVG 陽春占位封面（無 coverImage）的文章重生封面。
// 生圖走 lib/ai-image.mjs 的 generateImage（＝codex 原生 image_generation 唯一路徑，
// 站長 2026-08-22 裁示不得直打 gpt-image-2 API）。
// 風格：簡約編輯插畫；套台灣人鐵律 + 無文字。壓成 WebP 存 public/covers/，並寫回 frontmatter。
// 可重複執行（只挑沒有 coverImage 的，已處理的會跳過）。
import sharp from 'sharp';
import { readFileSync, writeFileSync, readdirSync } from 'node:fs';
import { join } from 'node:path';
import { generateImage } from './lib/ai-image.mjs';
...
async function genOne(file) {
  ...
  const gen = await generateImage({ topic: title, prompt, width: 1280 });
  const webp = await sharp(gen.buffer).webp({ quality: 82 }).toBuffer();
