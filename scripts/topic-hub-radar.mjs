 ...
 * 2026-08-08 站長複核後維持原設定，三件事都不改（下一個人不要再重新討論）：
 *   - **門檻 15 篇不降**：站上還有數百篇未歸主題，降門檻能開得更快，但薄中樞頁對 SEO 是反效果。
 *   - **每次最多 1 個不放寬**：自動上線的東西寧可慢，一次洗出多頁的風險翻倍。
 *   - **不改成按鈕核可**：把關已經全在機器這端（四道門檻＋文案逐條驗證＋build/check:links gate），
 *     真要撤掉就把 `src/content/topics/<id>.md` 的 status 改 inactive。
 * 成立訊息發**主題追蹤頻道**（不是 dev；dev 只留失敗），並在該頻道開一條 thread 追蹤後續。
 * 因為是自動上線，把關全部做在機器這端：
 ...
 *   2. 夠內聚：群內平均 TF-IDF 餘弦 ≥ MIN_COHESION（預設 0.045；環境變數 HUB_MIN_COHESION 可覆寫）
