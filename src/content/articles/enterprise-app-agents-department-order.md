...
tags:
  ...
  - "企業經營"
  - "AI治理"
coverImage: "covers/enterprise-app-agents-department-order-cover.webp"
...
### Gartner 預估 2026 年底企業應用內建 agent 的比例是多少？
Gartner 估到 2026 年底，40% 的企業應用會內建任務型 agent，2025 年這個比例還不到 5%，一年內成長超過八倍。

### 為什麼客服和服務台會是最先被 agent 化的部門？
因為這些部門本來就在接收請求、查資料、執行既定流程，例如查訂單、辦退費，幾乎是 agent 的標準題型，所以排在最前面。

### 法務和財務為什麼排在 agent 化順序的後段？
法務和財務的請求較不規則，判斷錯誤的代價也高，所以在客服、IT、業務支援、人資之後才輪到它們被 agent 化。

### agent 內建變成軟體預設配備後，企業真正要面對的難題是什麼？
是治理跟不上部署速度。導入單一 agent 容易，但要讓全公司同時用 agent 動到正式資料，權限、稽核、責任歸屬得同步到位，這件事比把 agent 裝起來難得多。

### 這股 agent 內建趨勢對新創公司有什麼意義？
過去要養一整組客服、IT、法務、財務團隊才有的能力，現在小團隊只要接上內建 agent 的軟體就能有雛形，等於取得大型公司等級的資源。
