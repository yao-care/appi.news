...
expertNote: "這件事對台灣做臨床或數位健康產品的團隊最直接：別急著把 AI 產出的機制推論當結論拿去做決策或產品宣稱。真正該投資的順序是先建乾淨可用的資料授權流程，再配懂行的人判讀，最後留一場真實驗證去對答案，少一環都白搭。"
risksAndLimits:
  - "模型的機制推論已在生物學上合理，但濕實驗尚未完成、也未走完正式同儕審查"
  - "相近的甘露糖回補故事先前曾放上 bioRxiv 預印本，無法完全排除模型訓練時已接觸相關線索"
  - "成功依賴專家提供乾淨資料集並能判讀輸出合理性，換一般使用者或雜訊資料未必複製"
references:
