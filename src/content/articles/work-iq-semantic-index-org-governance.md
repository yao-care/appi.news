...
tags:
  ...
  - "資料治理"
  - "AI agent"
coverImage: "covers/work-iq-semantic-index-org-governance-cover.webp"
...
### Work IQ 是什麼時候推出的？
微軟在 6 月 2 日的 Build 大會發表 Work IQ，6 月 16 日 API 正式上線，定位是讓 AI agent 讀懂公司怎麼運作的一層。

### Work IQ 索引的內容跟一般企業 AI 有什麼不同？
過去企業 AI 多半只翻檔案查資料，Work IQ 索引的是行為與關係，例如誰跟誰常開會、決策在哪些人之間流動、哪封信牽動哪個檔案。

### Work IQ 技術上怎麼運作？
微軟把上百個資料專用工具收斂成 10 個泛用工具，透過 MCP 漸進揭露，還提供 getSchema，讓 agent 執行當下就能查資料長什麼樣、怎麼組。

### 微軟設計了哪些權限防線？
每個請求都限定在使用者範圍內，agent 只能存取那位使用者原本看得到、做得到的東西；底層用 Rego 政策引擎依脈絡判斷規則，每次工具呼叫都留紀錄供稽核。

### 組織現在可以先盤點哪三件事？
第一，哪些協作行為該進索引、哪些不該，預設值別開到最大。第二，誰能查到誰的工作軌跡，這條界線誰在定。第三，查詢與組合權限由誰核給、有沒有留紀錄可回查。
