...
description: "零信任架構怎麼運作？拆解身分鑑別、設備鑑別、信任推斷三個階段，說明台灣政府機關與銀行等金融業導入進度，以及金管會從指引走到金融資安韌性發展藍圖，逐步納入自律規範的最新進度。"
...
<p>零信任架構（Zero Trust Architecture）分身分鑑別、設備鑑別、信任推斷三個階段推進，核心原則是不因為使用者在公司內網、設備是公司財產，就預設它值得信任，每一次存取都要重新驗證身分、設備狀態與當下情境。台灣政府機關已依官方時程分三階段導入；銀行等金融業則從 2024 年的鼓勵性指引，走到 2025 年底把零信任列進四年期的資安韌性藍圖，逐步變成自律規範，但目前仍不是一步到位的強制令。</p>
